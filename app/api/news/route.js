...
export async function GET() {
  try {
    ...
    const normalized = news.map((n) => ({
      ...n,
      _id: n._id.toString(),
      // Convert image object to string URL for compatibility with scroller component
      image:
...
export async function POST(req) {
  try {
    ...
    // Handle image - normalize to object format if present
    let imageData;
    ...
    // Attach any image metadata provided by the client (only create object if metadata exists)
    if (body.imagePublicId || body.imageTitle || body.imageDescription) {
      imageData = imageData || {};
      if (body.imagePublicId) {
        imageData.public_id = body.imagePublicId;
      }
      if (body.imageTitle) {
        imageData.title = body.imageTitle;
      }
      if (body.imageDescription) {
        imageData.description = body.imageDescription;
      }
    }
